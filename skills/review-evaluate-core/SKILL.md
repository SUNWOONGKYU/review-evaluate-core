...
> **v0.7 (2026-09-16) 역할 3분리 — 작성자 ≠ 리뷰어 ≠ Cross Validator.** 리뷰어는 의견(결함 목록)만 낸다. 수정은 그 산출물을 만든 **작성자**가 한다. 리뷰어가 반영 여부를 확인하고, Cross Validator가 채점한다. 리뷰어가 직접 고치면 엉뚱한 수정이 잦았던 것(사용자 지적)이 개정 이유다.
...
⚠️ **한 세션이 리뷰어와 작성자를 겸하지 못한다.** 대상이 1~2개인데 작성자(또는 작성자 대리)가 메인 세션이면, 메인 세션은 리뷰어를 맡지 않고 **리뷰어 서브에이전트 1명을 추가 투입**한다(리뷰어 1 + CV 1). 같은 세션이 찾고 고치면 v0.7이 막으려는 "엉뚱한 수정"이 그대로 재현된다.

### 작성자 결정 (v0.7)

| 산출물을 만든 주체 | 작성자 |
|---|---|
| 현재 살아 있는 서브에이전트·다른 세션 | 그 세션 (결함 목록을 보내 수정·회신 받음) |
...
```
메인 세션(Claude Code) — 파일 분류 + 배정 + 작성자 결정 + 결함 목록 전달 + Step 4 외부 자문 실행(codex·Gemini 호출 주체) + 최종 판단
│
├── Code Reviewer (subagent, sonnet, auto, subagent_type: code-reviewer)
│   담당: .py/.ts/.tsx/.js/.jsx/.sql 파일
...
│    → **CV=100 AND Round≥3 일 때만** 메인 세션이 codex + Gemini 호출(CLI/API/웹 구독 폴백) │
...
│                                                 │
│  판단:                                           │
│    - Round < 3         → 다음 라운드 계속 (CV=100이어도 3회 채움 — 이 규칙이 우선) │
│    - CV < 100          → 다음 라운드 계속 (외부 LLM 미호출) │
│    - CV = 100 AND Round ≥ 3 (최초 도달) → codex·Gemini 추가 자문 호출 │
...
│    - Round = 10        → 강제 종료 (현재 점수 보고 · 이 라운드에 CV=100이면 Step 4 실행 후 종료) │
│    - 연속 반영 0건 2회   → 자동 종료 (개선 불가 판단)  │
└─────────────────────────────────────────────────┘
```
...
[거부] {n}건
• {결함 요약} → 거부 이유 (한 줄)
[보류] {n}건 — 다음 라운드로 이월 (이유)   ※ 최종 보고의 "잔여" = 마지막 라운드까지 남은 보류·미확정 건
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
> ⛔ **진입 게이트 — CV<100이거나 Round<3이면 이 Step을 실행하지 마라.** 이 Step은 CV가 *이미 100*이고 *Round ≥ 3*일 때만 들어온다. 78·88·95·97·99 어느 점수든 100 미만이면 여기로 오지 말고 결함을 직접 고쳐 다음 라운드로. (상세: ABSOLUTE RULE의 ⛔ 박스)
...
| CV=100 최초 도달 (Round < 3) | = 100점 | ❌ (3회 채운 뒤 — Round<3 규칙이 우선) |
| CV=100 AND Round ≥ 3 (최초 도달) | = 100점 | **✅ codex·Gemini 각 1회 호출 (둘 다)** |
...
- **★ 웹 구독 폴백 (CLI·API 모두 막혔을 때 — 최후 수단, 2026-06 신설)**: codex CLI 거부·Gemini CLI 무료티어 폐지(IneligibleTierError)·gemini.cjs 키 전량 소진 등으로 API/CLI 경로가 다 막혀도, **로그인된 웹 구독 세션(chatgpt.com · gemini.google.com)을 컴퓨터 유즈로 몰아** 평가를 받는다. 웹 세션은 CLI 무료티어 폐지·API 키 소진과 **무관**(브라우저 로그인은 독립)이라, 사실상 무료로 외부 검증을 복구한다. 로그인된 웹 세션을 브라우저 자동화(CDP)로 재사용하는 원리다.
  ...
  - 웹 폴백 결과도 codex·Gemini와 동일한 v0.6 자문 규칙 적용(Critical/High만 반영, 점수·Medium·Low는 참고). 보고에 **"{ChatGPT/Gemini} 웹 구독 폴백"** 으로 경로 표기.
...
### Code 파일 (.py/.ts/.tsx/.js/.jsx/.sql) — 6기준 (배점: 내부 15점씩 + 사양 충족률 25점 = 100점)
...
| **사양 충족률** | 25 | 연관 문서 대비 구현 100% 일치 | GAP 건수별 구간 점수 (아래 상세 — 선형 아님) |
...
### Document 파일 (.md) — 6기준 (내부 15점씩 + 사양 충족률 25점 = 100점)

| 기준 | 만점 | 만점 조건 | 감점 기준 |
|------|:---:|----------|----------|
| **사실 정확성** | 15 | 오류 0건 | 사실 오류 1건당 -5 |
| **용어 일관성** | 15 | 프로젝트 규칙 100% 준수 | 용어 흔들림 1~3건 -5, 4건+ -10 |
| **구조 완성도** | 15 | 필수 섹션 전부 존재 | 필수 섹션 누락 1건당 -5 |
| **규칙 준수** | 15 | 금지 문구 0건 + 명칭 규칙 준수 | 위반 1건당 -5 |
| **실용성** | 15 | 즉시 활용 가능 | 따라 하다 막히는 지점 1건당 -3 |
| **사양 충족률** | 25 | 연관 문서/코드와 내용 일치 | GAP 건수별 구간 점수 (Code 루브릭과 동일) |

### Report 파일 (.html) — 6기준 (내부 15점씩 + 사양 충족률 25점 = 100점)

| 기준 | 만점 | 만점 조건 | 감점 기준 |
|------|:---:|----------|----------|
| **데이터 정확성** | 15 | DB 수치 100% 일치 | 수치 불일치 1건당 -5 |
| **렌더링 품질** | 15 | 태그 열림/닫힘 100% + CSS 정상 | 렌더 결함 1건당 -5 |
| **정식명칭·금지문구** | 15 | 위반 0건 | 위반 1건당 -5 |
| **구조 균일성** | 15 | 벤치마크 대비 ±10% 분량 | ±10% 초과 -5, ±25% 초과 -10 |
| **접근성** | 15 | 모바일 대응 + 인쇄 가능 | 미대응 항목 1건당 -5 |
| **사양 충족률** | 25 | 연관 문서/스크립트와 출력 일치 | GAP 건수별 구간 점수 (Code 루브릭과 동일) |
...
내부 5기준은 15점 만점, 사양 충족률은 25점 만점이다. 사양 충족률은 위 "사양 충족률 채점 상세"(25/20/15/10/5/0)를 그대로 쓰고, 15점 기준은 아래로 보간한다. **우선순위: 각 루브릭 표의 "감점 기준" 열이 먼저다** — 표에 있는 결함은 표대로 감점하고, 표에 없는 결함만 아래 구간으로 판단한다:

```
15점: 만점 조건 완벽 충족
13~14점: 기준표의 감점 항목에 해당하지 않는 경미한 미달 1~2건 (예: 주석 오탈자, 표기 흔들림)
10~12점: 기준표의 감점 기준 1건 적용 (예: 구문 오류 1건 -5 → 10점, 미구현 기능 1건 -3 → 12점)
6~9점: -3/건 기준 결함 2~3건(예: 미구현 기능 2건 → 9점) 또는 표에 없는 상당한 미달  ※ -5/건 기준 결함 2건은 산술대로 5점(아래 구간)
1~5점: 기준 대부분 미충족
0점: 해당 항목 전면 결함 (예: 실행 불가·보안 위반 다수)
```
...
📁 파일별 평가:
  [Code] script.py — XX/100점
    문법·실행: XX/15 | 보안: XX/15 | 품질: XX/15 | 테스트: XX/15 | 완성도: XX/15 | 사양 충족률: XX/25
  [Document] guide.md — XX/100점
    정확성: XX/15 | 용어: XX/15 | 구조: XX/15 | 규칙: XX/15 | 실용성: XX/15 | 사양 충족률: XX/25
  [Report] report.html — XX/100점
    데이터: XX/15 | 렌더링: XX/15 | 명칭: XX/15 | 균일성: XX/15 | 접근성: XX/15 | 사양 충족률: XX/25
...
> **Cross Validator 점수가 100이 되기 전에는, 그리고 Round 3을 채우기 전에는 codex를 호출하지 않는다.** 78·88·95·97·99점 — 그 무엇이든 100 미만이면 codex로 넘기지 말고 **그 결함을 작성자가 수정해 다음 라운드**로 간다. codex는 "CV=100을 *확정*하는 마지막 1회"이지, "개선 아이디어를 얻는 도구"가 아니다.
...
> - 따라서 **매 라운드 끝 체크**: "CV=100인가? 그리고 Round ≥ 3인가? 둘 중 하나라도 아니면 codex 부르지 말 것."
...
2. CV=100 도달 후(그리고 Round ≥ 3을 채운 뒤), codex(OpenAI/GPT)·Gemini(Google)를 **추가 자문**으로 각 1회 호출한다 — CV가 못 본 사각을 잡는 보조 검증(가치 큼). 가능하면 둘 다, 안 되면 가능한 쪽만(웹 구독 폴백 포함).
...
- 예외: Round = 10 도달(CV<100) OR 연속 반영 0건 2회 발생 시에만 미달 종료 허용 — 외부 LLM 미호출 상태 그대로 종료. (Round 10에서 CV=100 최초 도달은 미달이 아니라 "100점 달성" — Step 4 실행 후 [완료]만)
...
| 사유 | 조건 | 행동 |
|------|------|------|
| **100점 달성** | CV=100 AND Round ≥ 3 | ① Step 4 외부 자문(codex·Gemini 각 1회) 자동 실행 → ② 외부 C/H 0(또는 반영 후 CV 재확인 100) → ③ 최종 보고 + 사용자에게 [완료]/[추가] 확인. "[추가]" = 라운드(검토→작성자 수정→CV) 1회 추가, 외부 자문 재호출 아님. Round 10 달성이면 [완료]만(상한) |
| **10회 도달** | Round = 10 | 현재 점수로 강제 종료. 단 Round 10에서 CV=100에 처음 도달하면 "100점 달성" 경로가 우선 — Step 4 외부 자문을 실행하고 종료 |
| **개선 불가** | 연속 반영 0건 2회 | 자동 종료 |
...
⭐ 최종 평점: 100/100점
🔄 총 라운드: {N}회
📊 종료 사유: 100점 달성 (CV=100 · Round ≥ 3)
🌐 외부 추가 자문 (Step 4, 자동 실행 완료): codex XX/100 · Gemini XX/100 · Critical/High {n}건 → {0건 / 반영 후 CV 재확인 100} · {미실시 사유}

추가 검토·평가를 진행하시겠습니까? (외부 자문은 이미 끝났음 — [추가]는 내부 라운드 1회를 더 도는 것)
- [완료] 검토·평가 종료
- [추가] 1회 더 검토·평가 수행 (검토→작성자 수정→CV 채점, 외부 자문 재호출 없음) — 단 Round 10에서 달성한 경우 상한이라 [추가] 없이 [완료]만
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
⭐ 최종 평점: XX/100점
🔄 총 라운드: {N}회
📊 종료 사유: {10회 도달 / 연속 반영 0건 2회 — 개선 불가}
...
📁 파일별 최종 평가 (채점: Cross Validator — 리뷰어는 채점하지 않음):
  [Code] script.py — XX/100점
  [Document] guide.md — XX/100점
  [Report] report.html — XX/100점
  통합: XX/100점 (파일별 균등 평균)

📝 라운드별 수정 이력 (작성자: {작성자}):
  Round 1: 반영 {N}건 (Critical {n}, High {n}, Medium {n}, Low {n}) · 거부 {n}건(기각 {n}) · 이월 {n}건
  Round 2: 반영 {N}건 (…) · 거부 {n}건 · 이월 {n}건
  Round 3: 반영 0건 → 점수 유지
  ...
...
## Tools Required
- Read, Glob, Grep (검토용) + Bash 읽기 전용 실행(py_compile·빌드·렌더 확인 — 산출물 파일을 바꾸는 명령 금지) — 리뷰어·CV는 이 범위만
...
- PowerShell (Step 4 — codex CLI·gemini.cjs 호출. 호출 절차 스크립트는 PowerShell 문법이며 UTF-8 3종 강제가 PowerShell 변수라 Bash로 옮기지 않는다 — 메인 세션 전용)
- 브라우저 자동화 (Step 4 웹 구독 폴백 — Playwright `connect_over_cdp` 로 자동화 Chrome에 접속 — 메인 세션 전용)

## Execution Flow 요약

```
1. 평가 대상 결정 + 파일 타입 분류 (Code/Document/Report) + 서브에이전트 투입 판단 (3개 이상 → 풀 투입, 1~2개 → 메인 세션 검토 + Cross Validator 1명 — 단 메인 세션이 작성자면 리뷰어 서브에이전트 추가) + 작성자 결정 + 대상 확인 보고
2. Phase 1.5 — 연관 문서 탐색 → 사양-구현 대조 (GAP·EXTRA·MATCH → 사양 충족률 25점의 근거)
3. 순환 루프 시작 (최소 3회 · 최대 10회)
   Round N:
     ...
     e. 반영 0건이면 점수 유지
     f. 연속 반영 0건 2회 → 자동 종료
4. CV=100 AND Round ≥ 3 → Step 4 외부 자문 자동 실행 (codex·Gemini 각 1회, UTF-8 3종) → 외부 C/H만 작성자 반영 → CV 재확인
5. 최종 보고 → 사용자에게 [완료]/[추가] 확인 ("[추가]" = 내부 라운드 1회, 외부 재호출 없음)
6. 10회 도달(CV<100) 또는 연속 반영 0건 2회 → 미달 종료 (외부 자문 없이 현재 점수로 최종 보고). 10회에서 CV=100 최초 도달이면 4번 경로(Step 4 실행) 후 [완료]만
7. 최종 결과 보고 (파일별 점수 + 통합 점수 + 라운드별 반영·거부·이월 이력)
```
