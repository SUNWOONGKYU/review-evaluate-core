...
- **100점 = Cross Validator(Claude) 단계에서 달성한다.** Cross Validator가 100점을 주고 **최소 3라운드를 채운 뒤에만** 메인 세션이 codex·Gemini(웹 구독 폴백 포함)를 각 1회 추가 자문으로 호출해 사각 지대를 점검한다 — 100점을 무르는 게이트가 아니라, Critical/High 결함만 추가로 반영하는 보조 검증이다.
- 최대 10회, 또는 연속 반영 0건 2회 시 강제 종료한다. Round 10에서 CV=100에 처음 도달해도 "100점 달성" 경로가 우선한다.
...
|---|---|---|
| **Code** (.py/.ts/.tsx/.js/.jsx/.sql) | 문법·실행 · 보안 · 코드 품질 · 테스트·검증 · 기능 완성도 | 연관 문서 대비 구현 일치율 |
...
| v0.7 | 2026-09-17 | 8라운드 검토·외부 자문 반영 정리 — CV=100 확정에 최소 Round≥3 게이트 추가, Step 4(외부 자문) 실행 주체를 메인 세션으로 명확화, Round 10 도달 시 100점 달성 경로 우선 처리, 루브릭 보간표·최종 보고 템플릿 정리, 코드 확장자(.tsx/.jsx) 통일 |
